...
func GetAvailableSessions(user models.User, centers map[string]interface{}) []interface{} {
	...
	for _, center := range centers["centers"].([]interface{}) {
		...
		for _, session := range centerMap["sessions"].([]interface{}) {
			...
			if int(sessionMap["min_age_limit"].(float64)) <= user.Age && (strings.Compare(sessionMap["vaccine"].(string), string(user.PreferredVaccine)) == 0 || strings.Compare(string(user.PreferredVaccine), "ANY") == 0) && int(sessionMap["available_capacity"].(float64)) > 0 {
